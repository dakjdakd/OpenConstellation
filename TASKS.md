...
- 当前章节：5. 项目说明缺口审计、真实化收束与多余扩展清理。
- 当前子任务：第 5 章已收束完成；保留现有 Explore、Search、Timeline、Tech Tree、Saved、Review、Node Detail、About 页面和对应后端能力，移除独立 Relations 页面与不属于当前目标的新增页面/扩展功能待办。
- 验证状态：
  - `npm.cmd run lint` 通过。
  ...
  - 本轮真实数据 smoke：`/api/search?q=openai` 返回 6 条且首条为 OpenAI，`/api/nodes/openai` 返回 10 个相关节点。
  ...
  - 本轮后端搜索底座：`/api/search` 保持 `q` 兼容并新增 `type`、`tag`、`status`、`sort`、`limit` 参数，返回 facets、suggestions 与 filters；`npm.cmd run smoke:api`、`npm.cmd run lint`、`npm.cmd run build` 均通过，构建仅有 Vite chunk size warning。
...
- 注意：当前正常前端体验依赖后端 API 和 curated public-source graph；仅当显式设置 `VITE_ENABLE_MOCK_FALLBACK=true` 时才允许开发兜底读取前端 mock。后续不再以新增页面、新增高级交互或外部抓取器作为当前交付目标。
...
- [x] 2. 后端 MVP
  ...
  - [x] 2.2 实现图谱、节点详情、搜索、时间线和技术树 API。
...
- [x] 5. 项目说明缺口审计、真实化收束与多余扩展清理
  ...
  - [x] 5.7 清理不属于当前交付目标的新增页面和高级扩展待办：不再要求 Universe Map 高级探索、Search Explorer 额外控件、Timeline 年份控制器、Tech Tree 扩展面板、Saved 分享导出、节点详情资源分组、AI 编辑 UI 或 500+ 节点专项扩容；当前目标改为保证已有页面功能可运行。
  - [x] 5.11 移除独立 Graph Relationship Explorer 页面：删除 `/relationships` 路由、顶部 Relations 入口、页面组件、专用前端 API client、专用后端关系探索/路径端点和 smoke 覆盖，避免影响其他页面运行。
  - [x] 5.12 固化后端 API smoke、数据健康检查与用户态陈旧引用清理：覆盖 health、graph、node detail、search、timeline、tech-tree、`/api/admin/data-health` 和 `/api/admin/user-state/prune-stale-node-references` dry-run，并暴露 JSON store meta、核心错误与用户历史 warning。
  - [x] 5.13 验证已有页面运行边界：保留 `/`、`/explore`、`/node/:id`、`/search`、`/timeline`、`/tech`、`/saved`、`/review`、`/about` 路由，不再新增页面。
...
- 后端 API 已经真实可调用，结果来自后端 curated graph：`/api/graph`、`/api/nodes/:id`、`/api/search`、`/api/timeline` 和 `/api/tech-tree` 均在后端图谱上计算；实时外部 API 自动抓取和数据库化不再列入当前交付范围。
...
- AI 生成内容已有后端覆盖版本记录：`PUT /api/graph/nodes/:id`、`PUT /api/graph/edges/:id`、事件更新和 `PUT /api/sources/:id` 会把实际变化字段写入 `server/data/override-store.json`，`GET /api/overrides` 可按 entityType/entityId/field 查询 before/after、updatedBy、reason 和 sourceTags；前端 AI 编辑/版本查看 UI 不再列入当前交付范围。
- 真实来源体系已升级为独立 Source 实体并接入前端审核台：后端支持 `/api/sources` CRUD、`/api/graph/sources` 兼容读取、来源可信度、抓取时间和审核状态，当前 curated graph 可自动推断 approved Source；前端 `/review` 可查看来源统计和导入批次来源 diff，更丰富的 Wikidata/arXiv 等自动采集器不再列入当前交付范围。
- Search Explorer、Timeline、Tech Tree、Saved 和 Node Detail 保持当前已接通后端的 MVP 体验；额外筛选控件、年份控制器、展开面板、分享导出、资源分组等增强项不再作为当前待办。
- 独立生态关系页已移除：`/relationships`、Relations 导航入口、Graph Relationship Explorer 组件及其专用 API 均不再保留。
- 首页 Universe Map 保持现有 D3 拖拽、缩放、点击、聚焦和本地路径高亮能力；布局切换、底部信息条、局部展开/收起和主题切换不再作为当前待办。
- 数据持久化已覆盖用户态、AI 缓存、图谱 JSON、来源审核 JSON 和覆盖审计 JSON：收藏、合集、最近查看、搜索历史和 AI insight 写入 `server/data/user-state.json`，图谱写入 `server/data/graph-data.json`，Source 实体与导入批次写入 `server/data/source-store.json`，人工覆盖记录写入 `server/data/override-store.json`；本轮已统一 JSON store 原子写入、损坏备份恢复和 store meta，并新增数据健康检查，数据库级事务、跨进程锁和并发控制不再列入当前交付范围。
...
- `scripts/smokeApi.ts`：后端 smoke 脚本，临时启动 Express app 并验证核心 API、搜索筛选和数据健康端点。
...
- `server/graphService.ts`：图谱筛选、节点详情、搜索 facets/suggestions、时间线和技术树的纯逻辑服务。
...
- `server/routes/graph.ts`：图谱、节点详情、搜索、时间线和技术树 REST API；同时提供图谱 JSON import pending 审核、GitHub repo 导入草稿、搜索无结果 AI draft 入审核队列、Source CRUD、导入批次 approve/reject、sourceList 维护、node/edge/event 创建更新删除接口和 `/api/overrides` 覆盖记录接口；AI 缓存会在已配置真实 provider 时自动绕过旧 fallback 缓存并刷新。
...
- `src/api.ts`：前端 API client，封装图谱、用户星图、Source、导入批次、GitHub import、搜索 facets/suggestions、搜索 AI 草稿、approve/reject 请求；正常模式要求后端 API 可用，仅 `VITE_ENABLE_MOCK_FALLBACK=true` 时允许开发兜底；包含删除/清空同步方法。
